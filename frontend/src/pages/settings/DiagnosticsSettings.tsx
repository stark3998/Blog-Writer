...
export default function DiagnosticsSettings() {
  ...
  const doRun = async () => {
    ...
    try {
      const response = await runDiagnostics(
        { session_id: sessionId.trim() || undefined, include_billable: includeBillable, checks: flags },
        apiKey.trim()
      );
      setResult(response);
    } catch (err: any) {
  ...
  const statusChip = (status: string) => {
    if (status === "pass") return <span className="px-2 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700">PASS</span>;
    if (status === "warn") return <span className="px-2 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">WARN</span>;
    if (status === "skip") return <span className="px-2 py-1 rounded-full text-xs font-semibold bg-slate-100 text-slate-600">SKIP</span>;
  ...
  return (
    <>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Run health checks on connected services and API integrations.</p>
      </div>

      <section className="rounded-2xl p-6 border border-gray-200/70 bg-white mb-6">
        <div className="flex items-center gap-2 mb-4">
          <KeyRound className="w-4 h-4 text-emerald-600" />
          <h3 className="text-sm font-semibold text-gray-800">Run Controls</h3>
        </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <button onClick={() => toggleAll(!allSelected)} className="px-3 py-1.5 text-xs rounded-lg border border-gray-300 hover:border-emerald-300 hover:bg-emerald-50">
          ...
          {Object.entries(flags).map(([k, v]) => (
            <label key={k} className="inline-flex items-center gap-2 text-xs px-3 py-1.5 rounded-lg border border-gray-300 bg-white">
              <input type="checkbox" checked={v} onChange={(e) => setFlags((prev) => ({ ...prev, [k]: e.target.checked }))} />
              {k}
            </label>
          ))}
          <button
      ...
      {result && (
        <section className="rounded-2xl p-6 border border-gray-200/70 bg-white">
          <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${result.overall_status === "healthy" ? "bg-emerald-100 text-emerald-700" : result.overall_status === "degraded" ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700"}`}>
