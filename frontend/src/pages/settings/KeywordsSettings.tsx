...
export default function KeywordsSettings() {
  ...
  async function handleReset() {
    ...
    if (!confirm("Reset this topic's keywords to defaults? Your customizations will be lost.")) return;
    setResetting(true);
    try {
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage the keywords used for article relevance classification.</p>
        </div>
          ...
          <div className="w-52 flex-shrink-0 space-y-1.5">
            {topicList.map((t) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <Tag size={14} />
                  <span className="font-semibold">{topicLabels[t.topic] ?? t.topic}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="text-xs font-semibold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{t.keyword_count}</span>
                  {t.is_customized && <span className="w-1.5 h-1.5 rounded-full bg-amber-400" title="Customized" />}
                </div>
              </button>
            ))}
          </div>
            ...
            {detail && (
              <>
                <div className="flex items-center gap-3 mb-4">
                  <h3 className="text-lg font-bold text-gray-900">{topicLabels[detail.topic] ?? detail.topic}</h3>
                  {detail.is_customized && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-200/60 font-semibold">Customized</span>
                  )}
                  {isDirty && (
                  ...
                  <span className="text-xs text-gray-400">{editedKeywords.length} keywords</span>
                </div>
                ...
                {/* Keyword chips */}
                <div className="flex flex-wrap gap-2 max-h-[calc(100vh-26rem)] overflow-y-auto p-1">
                ...
                {filter && filteredKeywords.length === 0 && (
                  <p className="text-sm text-gray-400 mt-4">No keywords match "{filter}"</p>
                )}
              </>
            )}
          </div>
