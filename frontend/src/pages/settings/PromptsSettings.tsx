...
export default function PromptsSettings() {
  ...
  const testPlaceholders: Record<string, string> = {
    system_prompt: "Paste a sample URL analysis or article content to test blog generation...",
    editor_prompt: 'Paste blog content followed by an edit instruction, e.g.:\n\n---CONTENT---\n# My Blog Post\n...\n---INSTRUCTION---\nMake the tone more casual',
    linkedin_post_prompt: "Paste blog content to test LinkedIn post generation...",
    validation_agent_prompt: 'Paste content to validate, e.g.:\n\ncontent_type: linkedin_post\nblog_url: https://myblog.com/post\nsource_url: https://source.com/article\n\n--- GENERATED CONTENT ---\nYour post text here...\n\n--- SOURCE MATERIAL ---\nOriginal article text...',
    post_selector_prompt: 'Paste a JSON array of candidate posts, e.g.:\n[{"index":0,"title":"Post A","post_text":"...","article_url":"https://..."},{"index":1,"title":"Post B","post_text":"...","article_url":"https://..."}]',
  };
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Customize the AI prompts used for blog generation, editing, and LinkedIn posts.</p>
        </div>
          ...
          <div className="w-52 flex-shrink-0 space-y-1.5">
            {promptList.map((p) => (
              ...
              >
                ...
                {p.is_customized && <span className="flex-shrink-0 w-1.5 h-1.5 rounded-full bg-purple-400" title="Customized" />}
              </button>
            ))}
          </div>
            ...
            {detail && (
              <>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {detail.is_customized && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-purple-50 text-purple-600 border border-purple-200/60 font-semibold">Customized</span>
                  )}
                  {isDirty && (
                    <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-500 border border-blue-200/60 font-semibold">Unsaved changes</span>
                  )}
                </div>
                  ...
                  {testOpen && (
                    <div className="px-4 pb-4 space-y-3 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isDirty && <span className="text-xs text-blue-500 font-medium">Testing with unsaved editor content</span>}
                      </div>

                      {testError && (
                        <div className="p-3 rounded-lg bg-red-50 border border-red-200/60 text-red-600 text-sm">{testError}</div>
                      )}
                      ...
                      {testResult && (
                        <div className="space-y-2">
                          ...
                          <div className="rounded-xl bg-gray-900 border border-gray-800 p-3 max-h-64 overflow-y-auto">
                            <pre className="text-sm text-gray-200 whitespace-pre-wrap font-mono">{testResult.output}</pre>
                          </div>
                        </div>
                      )}
                    </div>
